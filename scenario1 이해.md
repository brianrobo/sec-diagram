...
정보 획득 후 단말을 다시 idle로 방출하는 이유는 뭐야?

공격자가 IMSI를 획득한 후 단말을 계속 붙들어 두지 않고 서둘러 RRC_IDLE 상태로 방출(Release/Reject)하는 데에는 아주 영리하고도 치명적인 이유들이 숨어 있습니다.

주요 이유는 크게 3가지로 요약할 수 있습니다.

1. 공격의 은폐 (Stealthiness)
가짜 기지국(FBS)은 실제 통신 서비스(인터넷, 전화 등)를 제공할 수 있는 코어망(Core Network)이 없습니다.

문제: 단말이 계속 연결(Connected) 상태에 있으면, 단말은 데이터를 주고받으려 시도하게 됩니다. 하지만 FBS는 이를 처리하지 못하므로 단말에 곧 '서비스 불능' 아이콘이 뜨거나 사용자가 이상을 감지하게 됩니다.

해결: 정보를 얻자마자 바로 IDLE로 쫓아내면, 단말은 다시 정상 기지국을 찾아 떠나게 됩니다. 사용자는 아주 잠깐 통신이 끊겼다가 다시 붙은 것으로 생각하여 공격당했다는 사실을 인지하지 못합니다.

2. 다음 타겟 확보 (Efficiency & Scalability)
FBS는 하드웨어 자원이 한정적입니다.

리소스 관리: 수많은 단말을 동시에 연결 상태로 유지하려면 막대한 무선 자원(Radio Resource)과 프로세싱 능력이 필요합니다.

순환 구조: 한 단말의 IMSI를 따자마자 바로 방출해야, 그 자리에 다른 단말을 유인하여 새로운 정보를 수집할 수 있습니다. 즉, **'IMSI 탈취 컨베이어 벨트'**를 가동하는 것입니다.

3. 단말의 의심 차단 (Avoiding Radio Link Failure)
단말은 기지국과 연결된 상태에서 일정 시간 동안 데이터 응답이 없으면 **Radio Link Failure(RLF)**를 선언하고 시스템 로그를 남깁니다.

사후 추적 방지: 단말이 RLF 로그를 남기면 나중에 정상 망에 접속했을 때 이 비정상적인 기록을 보고하게 됩니다.

정당한 거절: 대신 FBS가 "네트워크가 바쁘니 나중에 다시 시도해(Cause #11: PLMN not allowed)" 같은 정당해 보이는 거절 메시지(Reject)를 보내면, 단말은 스펙에 규정된 절차대로 조용히 물러나게 됩니다.
